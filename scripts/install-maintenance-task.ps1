...
.DESCRIPTION
  ...
  on a daily cadence (default: 03:00) plus once shortly after each logon
...
.PARAMETER SweepDays
  Forwarded to repo-maintenance.ps1 (-SweepDays). Default 7.

.PARAMETER MaxTargetGB
  Forwarded to repo-maintenance.ps1 (-MaxTargetGB). Default 4 per Cargo
  target directory.
...
$argline = "-NoProfile -WindowStyle Hidden -NonInteractive -ExecutionPolicy Bypass -File `"$maintScript`" -Apply -Root `"$scanRoot`" -SweepDays $SweepDays -MaxTargetGB $MaxTargetGB"
$action  = New-ScheduledTaskAction -Execute $pwsh -Argument $argline

# Daily trigger. A logon trigger would require elevation to register
...
$tDaily = New-ScheduledTaskTrigger -Daily -At $At
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $tDaily `
    -Settings $settings `
...
Info "Installed '$TaskName': runs daily at $At (catches up via -StartWhenAvailable if the PC was off)."
