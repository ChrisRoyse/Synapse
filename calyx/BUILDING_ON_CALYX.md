...
- **A new gate ships with its migration, or with its ledger.** A structural refusal added to an admission path must either migrate **every** built‑in call site in the same change, or land an explicit **shrinking ledger** of what is still non‑compliant (`RECORD_VECTOR_MAGNITUDE_GRANDFATHERED` is the worked example: a list that may only get shorter, each entry naming its panel and row count). A gate with neither is not a safeguard — it is a latent outage waiting for the next process restart, because admission gates fire on maintenance hot paths rather than in front of a developer. #1963 shipped without either and took recall to zero on the next unrelated deploy; #1964 shipped the same class of gate with a ledger and cost nothing.
- **A partial view must refuse, not answer.** Any handle that opens a subset of the store — selected column families, a latest‑only restore, a sampled panel — must remember that decision and fail closed when read outside it, naming the subset. Answering "empty" for "I cannot see this" is the one wrong answer no caller can detect, and it is *worse* in reverse: a readback confirming a purge reads zero and reports success.

**Anti‑patterns (refuse):** flattening the panel · embedding an already‑explicit value · selling associations past the ceiling · labeling ungrounded output "trusted" · mutating a frozen lens · making lens plug‑in harder · bolting on a separate search/graph/vector DB · a side store as a source of truth · a harness standing in for real verification · **a maintenance sweep that aborts globally when one item fails** (isolate per item, record the failure against it, continue — the active generation must never be collateral damage from a parked one) · **a provenance field with a plausible default** (`build: "dev"` from an env var nothing sets is indistinguishable from a real answer).
