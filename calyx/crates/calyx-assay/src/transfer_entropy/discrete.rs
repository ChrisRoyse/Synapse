...
mod tests {
    ...
    fn sample(future: f32, own: f32, source: f32) -> LaggedSample {
        LaggedSample {
            future: vec![future],
            joint_past: vec![source, own],
            own_past: vec![own],
        }
    }

    /// `Yf == Xp` and `Yp` independent: TE is exactly `H(Yf)` = 1 bit for a
    /// balanced binary source.
    #[test]
    fn copy_channel_reaches_one_bit() {
        let mut samples = Vec::new();
        for index in 0..400 {
            let source = (index % 2) as f32;
            let own = ((index / 2) % 2) as f32;
            samples.push(sample(source, own, source));
        }
        let streams = symbolize("test", &samples).expect("symbolize");
        let indices: Vec<usize> = (0..samples.len()).collect();
        let bits = te_bits(&streams, &indices);
        assert!(
            (bits - 1.0).abs() < 0.02,
            "expected ~1 bit for a copy channel, got {bits}"
        );
        assert_eq!(distinct_count(&joint_codes(&streams, &indices)), 4);
    }

    /// Independent coordinates: the Miller-Madow correction cancels the plug-in
    /// bias, so the estimate clamps to 0 rather than inventing a signal.
    #[test]
    fn independent_streams_report_zero() {
        let mut samples = Vec::new();
        for index in 0..800 {
            let future = (index % 2) as f32;
            let own = ((index / 2) % 2) as f32;
            let source = ((index / 4) % 2) as f32;
            samples.push(sample(future, own, source));
        }
        let streams = symbolize("test", &samples).expect("symbolize");
        let indices: Vec<usize> = (0..samples.len()).collect();
        let bits = te_bits(&streams, &indices);
        assert!(bits < 0.01, "expected ~0 bits for independence, got {bits}");
    }

    /// Below the per-state quorum the estimator refuses instead of returning a
    /// number.
    #[test]
    fn state_quorum_refuses() {
        let mut samples = Vec::new();
        for index in 0..12 {
            let value = (index % 6) as f32;
            samples.push(sample(value, value, value));
        }
        let error = symbolize("test", &samples).expect_err("quorum must refuse");
        assert_eq!(error.code, CALYX_TE_DISCRETE_STATE_QUORUM);
    }
}
