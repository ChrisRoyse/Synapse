//! `hygiene_report` tool integration FSV (#874): real daemon, real `RocksDB`,
//! real MCP calls, real derivation pipeline. Plants an injection string in a
//! five-day morning routine (outlook → excel → teams) — the injection rides
//! the excel window title — segments the timeline with the real
//! `episode_segment` tool, mines a routine with `routine_mine`, scans storage
//! with `hygiene_scan_storage`, then asserts `hygiene_report` traces each
//! flagged `CF_TIMELINE` row forward to the episodes it fed and the routine
//! mined from them. Covers honest-empty, source/score/time filters, paging,
//! operator lifecycle surfacing, the non-timeline-source note, validation
//! errors, and a post-shutdown physical readback that re-derives the
//! flag → episode → routine chain straight from the column families.
...
#[allow(
    ...
    reason = "one ordered happy-path + edge-case FSV narrative against a single live daemon"
)]
async fn hygiene_report_links_flags_to_episodes_and_routines() -> anyhow::Result<()> {
    ...
    // CORE: report traces flags → episodes → the mined routine.
    let report = structured(&client.tools_call("hygiene_report", json!({})).await?)?;
    println!(
        "readback=hygiene_report core summary={} scanned_flag_rows={} scanned_episode_rows={} scanned_routine_rows={}",
        report["summary"],
    ...
    for flag in flags {
        ...
        assert_eq!(
            flag["flag"]["record"]["source_cf"], cf::CF_TIMELINE,
    ...
    assert_eq!(obs_flags[0]["derived_episodes"].as_array().map_or(1, Vec::len), 0);
    assert_eq!(obs_flags[0]["derived_routines"].as_array().map_or(1, Vec::len), 0);
    ...
    let cursor = p1["next_cursor"].as_str().context("next_cursor")?.to_owned();
        ...
        &client
            .tools_call(
                ...
                json!({"routine_id": routine_id.clone(), "action": "confirm",
                       "note": "fsv: confirmed, now poisoned-aware"}),
            )
            .await?,
    )?;
    ...
    assert!(any_confirmed, "confirmed lifecycle must surface in the report");
    assert_eq!(after_confirm["summary"]["impacted_confirmed_routine_count"], 1);
    ...
    for bad in [
        ...
        let invalid = client.tools_call_error("hygiene_report", bad.clone()).await?;
    ...
    assert!(!covering.is_empty(), "a physical episode must cover the flag ts");
    assert!(
        covering
            .iter()
