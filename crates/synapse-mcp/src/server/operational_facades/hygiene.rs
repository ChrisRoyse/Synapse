    ...
    match operation {
        ...
        HygieneOperation::GuardCalibrate => {
            ...
            Ok(Json(hygiene_response(
                operation,
                format!(
                    "guard_id={} adjudicated_good={} adjudicated_bad={} unadjudicated={} incomplete_guarded_slots={} policy={} policy_bad_accepts={} policy_far={:.6} policy_frr={:.6} policy_tail={:.6} policy_certified={} slots={} persisted={} readback_calibrated={} guard_cf_profile_bytes={}",
                    response.guard_id,
                    response.adjudicated_good,
                    response.adjudicated_bad,
                    response.unadjudicated,
                    response.adjudicated_incomplete_guarded_slots,
                    response.policy,
                    response.policy_bad_accepts,
                    response.policy_achieved_far,
                    response.policy_achieved_frr,
                    response.policy_clopper_pearson_tail,
                    response.policy_certified,
                    response.slots.len(),
                    response.persisted,
