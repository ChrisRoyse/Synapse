...
impl SynapseService {
    #[tool(
        description = "Public storage facade for the <=40 MCP surface. operation=inspect/summary are read-only Calyx vault reports; operation=snapshot_gc_status is an O(1) independent read of the live vault's physical MVCC reclamation counters; operation=anchors decodes physical Calyx Anchors CF rows for an exact source row; operation=row_read returns ONE exact physical CF_OBSERVATIONS row through its typed decoder; operation=search_rebuild rebuilds a persisted Calyx search generation; operation=transcript_order_status independently compares the complete retained CF_AGENT_TRANSCRIPTS set to CF_AGENT_TRANSCRIPT_ORDER and returns digests plus a revision token; operation=transcript_order_rebuild is maintenance-gated and accepts only that exact token, persists crash-recovery intent, rebuilds the derived projection, fully reconciles it, and publishes readable metadata; operation=panel_lifecycle reads or maintenance-gates revision-guarded Registry mutations; operation=gc_once, backup, and other state-changing operations are maintenance-gated and return physical readback. Synthetic probe-row writes are debug-only raw routes and are not part of this production facade. Unknown operations and mismatched operation payloads fail closed."
    )]
