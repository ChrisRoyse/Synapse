...
impl SynapseService {
    #[tool(
        description = "Public storage facade for the <=40 MCP surface. operation=inspect/summary are read-only Calyx vault reports; operation=anchors decodes physical Calyx Anchors CF rows for an exact source row; operation=row_read returns ONE exact physical row body decoded through the typed record its writer used (readable CF allowlist: CF_OBSERVATIONS only; addressed by key_hex or observation_id, both returned under `diagnostics.persisted` on every observe response; read-only, READ_STORAGE, never maintenance-gated), reporting the stored value's exact length and SHA-256 plus the decoded non-content fields verbatim, with content-bearing strings reduced to SHA-256 digests and lengths and collection members omitted — a non-allowlisted CF, a missing row, an oversized value, or an undecodable value all fail closed and no raw row bytes are returned under any circumstances; operation=search_rebuild rebuilds and independently reopens the exact-panel persisted search generation; operation=panel_lifecycle reads or maintenance-gates revision-guarded add/park/unpark/retire mutations in the physical Registry CF; operation=gc_once is maintenance-gated and returns separate CF row-count/readback evidence; operation=backup is maintenance-gated and builds a consistent, self-verifying durable vault backup behind backup_in_progress.json, then publishes by writing the verified SHA-256 manifest and removing that marker; operation=backup_status independently reads target/marker/manifest state when a long backup outlives a client call; operation=restore_verify refuses in-progress targets and performs read-only byte-level verification of a published vault directory. Mutating operations require the maintenance capability. Synthetic probe-row writes are debug-only raw routes and are not part of this production facade. Unknown operations and mismatched operation payloads fail closed."
    )]
