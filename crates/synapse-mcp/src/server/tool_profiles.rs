...
#[cfg(test)]
mod facade_schema_parity_tests {
    use super::*;

    /// The #1886 gate, run against the real router the daemon builds.
    ///
    /// This is a compile-time-cheap echo of the startup assertion, not the
    /// acceptance surface: the enforcement surface is
    /// `ImmutableToolSurface::from_tool_router`, which refuses to construct the
    /// service. Keeping the same call here means a drift can be seen without
    /// launching a daemon, and the failure prints the exact symmetric
    /// difference in both directions.
    #[test]
    fn facade_contract_operations_match_live_schema() {
        let tools =
            super::super::schema_sanitize::sanitize_tools(SynapseService::tool_router().list_all());
        let parity = inspect_facade_schema_parity(&tools, FACADE_TOOL_CONTRACTS);
        assert!(
            parity.verified,
            "{}\nfull verdict: {}",
            parity.startup_failure_message(),
            serde_json::to_string_pretty(&parity).unwrap_or_else(|error| error.to_string())
        );
        assert_eq!(
            parity.matched_tool_count,
            FACADE_TOOL_CONTRACTS.len(),
            "every declared facade must have been compared, not skipped"
        );
    }

    /// Proves the gate reads a real enum out of a real schema rather than
    /// trivially agreeing.
    ///
    /// `storage` is the #1886 evidence case: its contract declared 5 operations
    /// while `StorageOperation` had 13. The live side of the comparison must
    /// therefore come back with all 13 names, resolved through the `$defs`
    /// entry `schemars` generated for the enum — a gate that could not do this
    /// is the gate that let those 8 drift.
    #[test]
    fn the_storage_facade_resolves_its_live_operation_enum_from_the_served_schema() {
        let tools =
            super::super::schema_sanitize::sanitize_tools(SynapseService::tool_router().list_all());
        let storage = tools
            .iter()
            .find(|tool| tool.name.as_ref() == "storage")
            .expect("the storage facade must be registered");

        let LiveOperationSchema::Enumerated {
            schema_name,
            operations,
        } = live_operation_schema(storage)
        else {
            panic!("storage must publish a closed operation enum in its input schema");
        };
    ...
    /// property, which is what lets the sentinel mean something.
    #[test]
    fn a_flat_facade_publishes_no_operation_property() {
        let tools =
            super::super::schema_sanitize::sanitize_tools(SynapseService::tool_router().list_all());
        let find = tools
            .iter()
            .find(|tool| tool.name.as_ref() == "find")
            .expect("the find facade must be registered");

        assert_eq!(live_operation_schema(find), LiveOperationSchema::Flat);
    }
}
