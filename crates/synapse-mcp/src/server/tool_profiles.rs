...
const FACADE_SCHEMA_PARITY_REMEDIATION: &str = "add an op(...) entry to that facade's FACADE_TOOL_CONTRACTS block for every `live_but_undeclared_in_schema_enum` operation and delete every `declared_but_absent_from_schema_enum` entry; a flat facade must carry FLAT_FACADE_OPERATION_ENUM and expose no `operation` property at all";
