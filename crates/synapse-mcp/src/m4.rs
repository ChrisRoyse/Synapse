...
/// `cli` and the spawn fields) or a template-rendered spawn (set `template_id`
/// and `template_params`). The two are mutually exclusive: when a template is
/// used, the template fully defines `cli`/`model`/`prompt`/`working_dir`/`target`
/// and supplying any of them is a loud error — an instance is rendered
