...
| `calyx-assay` | bits engine: mutual information via **two instruments chosen from the measured column** — KSG for a genuinely continuous column, a Miller-Madow-corrected contingency-table plug-in for a categorical one (`mi_estimator`, #1672) — plus sufficiency, redundancy/effective rank, transfer entropy, interaction info, Lomb–Scargle periodicity, CUSUM, MMD drift, Bayesian posteriors, Granger/CCM, admission contract |
