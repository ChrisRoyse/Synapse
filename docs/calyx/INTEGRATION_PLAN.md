...
| `calyx-assay` | bits engine: mutual information via **three instruments chosen from the measured column** — KSG for a genuinely continuous column, a Miller-Madow-corrected contingency-table plug-in for a supported categorical table, and a calibrated held-out logistic lower bound for sparse multivariate binary lanes (`mi_estimator`, #1672/#1690) — plus sufficiency, redundancy/effective rank, transfer entropy, interaction info, Lomb–Scargle periodicity, CUSUM, MMD drift, Bayesian posteriors, Granger/CCM, admission contract |
...
1. **In-process embedding, one durable vault.** `synapse-storage` owns the sole `AsterVault` at the configured daemon DB path (default `%LOCALAPPDATA%\synapse\db-daemon\`). M3 lifecycle and health read that exact storage-owned handle; no side vault exists. No daemon-to-daemon hop; Calyx runs inside `synapse-mcp.exe`.
