...
5. Missing and stale refusals finish the denied call lifecycle, emit `notifications/tools/list_changed`, and then run full session teardown. A Streamable HTTP tool response may already be committed as SSE before asynchronous teardown completes, so the server does not claim it can retroactively replace that in-flight response. The HTTP middleware instead intercepts the next request carrying the terminated session id before RMCP dispatch and returns HTTP 404 with the exact missing/stale root-cause code. This is the MCP 2025-11-25 recovery boundary: the specification permits server termination at any time and requires a client receiving 404 for its session id to initialize a new session. The server never admits the stale call or fabricates an attestation.
...
7. The registry's current request must be a later `tools/call:<name>`. This proves the attested `tools/list` and a subsequent call were observed on the same initialized session before caller-current status is reported.
...
- A client that ignores discovery notifications cannot remain permanently wedged in a valid-but-untrusted session: the first unsafe call expires that session and the next protocol cycle must initialize and list tools again.
