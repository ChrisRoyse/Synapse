...
5. **Directional predictive dependence** — transfer entropy and Granger measure whether one stream's past adds information about another stream's future. This gives a time-directed observational arrow, not an identified intervention effect; structural causal claims additionally require an explicit estimand and identification conditions.
...
- **Transfer entropy** `T(A→B) = I(B_future; A_past, B_past) − I(B_future; B_past)`, with a lag sweep; direction = the larger of `T(A→B)`, `T(B→A)`. Under Gaussian assumptions this is equivalent to Granger causality, so agreement between those two lanes is not independent confirmation.
- **Causal evidence maps keep unlike evidence unlike.** For every stream pair, preserve transfer entropy, linear Granger, signed lag correlation, nonlinear CCM, and point-process co-intensity as separate lanes; preserve PC-stable/partial-correlation skeletons and the multivariate Hawkes branching graph at map scope. Correct each p-value family for multiplicity. Observational artifacts are labeled `observational_predictive`; only an explicit persisted intervention/exposure contract with consistency, exchangeability, positivity, and the declared interference assumptions may justify `structural_identified`.
...
4. **Find causal/temporal structure.** timestamps + temporal lenses + recurrence series → exhaustive typed causal map (`C(n,2)` stream pairs, explicit time scope, FDR correction) → periodicity + hazard (scheduling) → next-occurrence prediction. Treat the map as observational predictive evidence unless a separate identification contract supports a structural effect.
