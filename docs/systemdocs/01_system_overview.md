...
| Windows APIs | `windows` crate (Win32 Foundation, UI Automation, HiDPI, WindowsAndMessaging, Input KeyboardAndMouse/XboxController, GDI/DWM, JobObjects, Registry, StationsAndDesktops, …) | 0.62.2 |
...
Data lives under `%LOCALAPPDATA%\synapse` (db, logs, models, replays, runs); the daemon auth token at `%APPDATA%\synapse\token.txt` (see [03_configuration.md](03_configuration.md)). The store is a Calyx vault with **17 logical column-family collections**; values are JSON; storage `SCHEMA_VERSION = 1`. Full schema in [04_storage_and_persistence.md](04_storage_and_persistence.md).

| Tier | What | Behavior |
|---|---|---|
| Sacred | user profiles (TOML on disk), registered models | not regenerated by the system |
| Regenerable | Calyx-backed timeline/episodes/agent events, OCR/detection caches | Calyx retention/GC + 5-level disk-pressure write-shedding |
| Ephemeral | in-memory rings (audio 30 s), capacity-1 demand-driven frame channel, replay temp | dropped on restart |
...
|---|---|---|---|
| **Capture** (`synapse-capture`) | Windows screen/window capture | GDI `BitBlt` reads physical visible-desktop pixels into owned BGRA8 memory. The loop preflights synchronously, uses a capacity-1 demand signal and a 250 ms floor, and records terminal worker errors. Hidden, minimized, cloaked, partly off-screen, or unsupported WGC/DXGI semantics fail with typed errors; no alternate backend is selected. Windows/the display driver may still accelerate GDI internally. Windows-only; non-Windows fails loud. | [05](05_capture_subsystem.md) |
...
| **Profiles** (`synapse-profiles`) | Per-application profile matching | TOML profiles, `deny_unknown_fields`; foreground-window match ranked exe > title_regex > steam_appid > window_class, tie-break newest mtime; 200 ms debounced hot-reload, fail-open. | [11](11_profiles_subsystem.md) |
| **Models** (`synapse-models`) | ONNX model registry/verify/session | 1 registered model (RT-DETRv2-S COCO); SHA-256 streaming verify (64 KiB chunks); the installed daemon enables ORT's CPU EP only (no CUDA/DirectML provider); downloads disabled in M1 (manual side-load to `%LOCALAPPDATA%\synapse\models`). | [13](13_models_subsystem.md) |
